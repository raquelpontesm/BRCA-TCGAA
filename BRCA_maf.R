...
brca.clinical$Overall_Survival_Status <- 1
brca.clinical$Overall_Survival_Status[which(brca.clinical$vital_status == "Dead")] <- 0
brca.clinical$time <- brca.clinical$days_to_death
brca.clinical$time[is.na(brca.clinical$days_to_death)] <- brca.clinical$days_to_last_follow_up[is.na(brca.clinical$days_to_death)]
...
## Visualization ----
# CHECAR: nao tem nenhuma indel?? 
plotmafSummary(maf = brca.maf, rmOutlier = TRUE, addStat = 'median', dashboard = TRUE, titvRaw = FALSE)

# Oncoplots
# pesquisar os top 20 no genecards relacionados a qualquer tipo de cancer
oncoplot(maf = brca.maf, top = 15)
...
# Lollipop
lollipopPlot(maf = brca.maf, gene = 'PIK3CA', AACol = 'HGVSp_Short', showDomainLabel = FALSE)
lollipopPlot(maf = brca.maf, gene = 'PIK3CA', AACol = 'HGVSp_Short', showMutationRate = TRUE)

# rainfall plots
rainfallPlot(maf = brca.maf, detectChangePoints = TRUE, pointSize = 0.4)

# Compare mutation load against TCGA cohorts
brca.mutload = tcgaCompare(maf = brca.maf, cohortName = 'example-brca', logscale = TRUE, capture_size = 50)
...
gisticBubblePlot(gistic = brca.gistic) # Error in plot.new():figure margins too large

gisticOncoPlot(gistic = brca.gistic, clinicalData = getClinicalData(x = brca.mafclin), clinicalFeatures = 'ajcc_pathologic_stage', sortByAnnotation = TRUE, top = 10)   # ERRO


## Analysis 

# Somatic interations ----
somaticInteractions(maf = brca.maf, top = 10, pvalue = c(0.05, 0.1))

# Detecting cancer driver genes based on positional clustering ----
brca.sig = oncodrive(maf = brca.mafclin, AACol = 'HGVSp_Short', minMut = 5, pvalMethod = 'zscore')
plotOncodrive(res = brca.sig, fdrCutOff = 0.1, useFraction = TRUE, labelSize = 0.5)

# Adding and summarizing pfam domains ----
brca.pfam = pfamDomains(maf = brca.maf, AACol = 'HGVSp_Short', top = 10)

# Protein summary (Printing first 7 columns for display convenience)
brca.pfam$proteinSummary[,1:7, with = FALSE]

# Domain summary (Printing first 3 columns for display convenience)
brca.pfam$domainSummary[,1:3, with = FALSE]
...
fab.ce$groupwise_comparision[p_value < 0.05] #Error in plot.new(): figure margins too large

plotEnrichmentResults(enrich_res = fab.ce, pVal = 0.05, geneFontSize = 0.5, annoFontSize = 0.6)

# Oncogenic Signaling Pathways ----
OncogenicPathways(maf = brca.mafclin)

PlotOncogenicPathways(maf = brca.mafclin, pathways = "RTK-RAS")
