...
# checking 
getSampleSummary(luad.gistic) # xx samples (Tumor_Sample_Barcode) 
getGeneSummary(luad.gistic) # xx genes (hugo)
getCytobandSummary(luad.gistic) # xx cytobands

write.GisticSummary(gistic = luad.gistic, basename = 'luad.gistic')
...
# oncoplot 
gisticOncoPlot(gistic = luad.gistic, clinicalData = getClinicalData(x = luad.mafclin), clinicalFeatures = 'ajcc_pathologic_stage', sortByAnnotation = FALSE, top = 10)

gisticOncoPlot(gistic = luad.gistic, top = 20)
